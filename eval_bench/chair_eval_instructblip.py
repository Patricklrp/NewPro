...
def main():
    ...
    for batch_id, data in enumerate(chair_loader):
        ...
        if args.use_ritual:
            ...
            with torch.inference_mode():
                outputs = model.generate(
                    {"image": image.cuda().half(), "prompt": qs_desc},
                ...
                logger.info(f"D: {outputs}")
            
                # raw_image.save('raw_image.png')
                raw_image_neg = generate_image_stable_diffusion(pipe, outputs)
                # raw_image_neg.save('image_neg.png')
                image_neg = vis_processors['eval'](raw_image_neg)
                image_neg = torch.tensor(image_neg).unsqueeze(0)      
        ...
        now = time.time()
        if (batch_id + 1) == effective_total or (now - last_log_time) >= 30:
